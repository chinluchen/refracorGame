...
@onready var exam_array_ui_os = get_tree().get_root().find_child("ExamArrayUIOS", true, false)
...
func _on_input_event(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.pressed:
		...
		var label = action_labels[current_index]
		...
		# ✅ 加入 log（OS）
		ActionLogger.log_action("OS：" + label, "exam_turn")
